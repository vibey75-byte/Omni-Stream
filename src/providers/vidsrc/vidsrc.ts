...
export class VidSrcProvider extends BaseProvider {
    ...
    private async getSources(
        ...
        try {
            ...
            if (!html) return this.emptyResult('Failed to fetch page', media);

            const secondUrl = this.extractSecondUrl(html);
            if (!secondUrl) return this.emptyResult('Invalid or expired token', media);

            const secondHtml = await this.fetchPage(secondUrl.url, media);
            if (!secondHtml) return this.emptyResult('Failed to fetch stream page', media);

            const thirdUrl = this.extractThirdUrl(secondHtml, secondUrl.url);
            if (!thirdUrl) return this.emptyResult('Failed to extract stream URL', media);

            const thirdHtml = await this.fetchPage(thirdUrl.url, media);
            if (!thirdHtml) return this.emptyResult('Failed to fetch final stream page', media);
    ...
    private emptyResult(
        ...
        return {
            ...
            diagnostics: [
                {
                    code: 'PROVIDER_ERROR',
                    message: `${this.name}: ${message}. Note that VidSrc blocks all kinds of VPN IPs.`,
                    field: '',
                    severity: 'error'
                }
            ]

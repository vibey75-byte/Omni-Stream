...
export class IcefyProvider extends BaseProvider {
            ...
            if (!response.ok) {
                throw new Error(
                    `API request failed with status ${response.status}` +
                        (response.status === 403
                            ? ` (probably blocked by Cloudflare. If you are running it locally, try going to ${this.BASE_URL} and solving the CAPTCHA manually.)`
                            : '')
                );
            }

            const data = (await response.json()) as { stream: string };
    ...
    private buildApiUrl(media: ProviderMediaObject): string {
        if (media.type === 'movie') return `${this.BASE_URL}/movie/${media.tmdbId}`;
        if (media.type === 'tv') {
            if (!media.s || !media.e) throw new Error('Missing season or episode');

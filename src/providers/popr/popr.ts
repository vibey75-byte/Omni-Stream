...
export class PoprProvider extends BaseProvider {
    ...
    async getMovieSources(media: ProviderMediaObject): Promise<ProviderResult> {
        try {
            const movieSource = await this.fetchSource(media, 'movie');
            return { sources: movieSource.sources, subtitles: movieSource.subtitles, diagnostics: [] };
        } catch (error) {
    ...
    async getTVSources(media: ProviderMediaObject): Promise<ProviderResult> {
        try {
            const tvSource = await this.fetchSource(media, 'tv');
            return { sources: tvSource.sources, subtitles: tvSource.subtitles, diagnostics: [] };
        } catch (error) {
            return this.emptyResult(
                error instanceof Error ? error.message : 'error at getting source',
                media
    ...
    private async fetchSource(
        ...
        const requests = servers.map((server) =>
            fetch(buildUrl(server), { headers: this.HEADERS }).then(async (res) => {
                ...
                const { isValid, type: streamType } = await this.checkStreamType(
                    stream.url, streamHeaders, server
                );
                ...
                const proxyHeaders = { ...this.HEADERS, ...streamHeaders };
                ...
                return {
                    source: {
                        ...
                        audioTracks: [
                            {
                                language: languages ? quality.toLowerCase().slice(0, 3) : 'eng',
        ...
        return { sources, subtitles: Array.from(subtitlesMap.values()) };
    }
